...
.Suggestion Service Method/URI Definitions


=== Technology Stack ===
Below is a list of technologies used in the development of Suggestion.
...
Install the following tools if you do not already have them:
 ...
 . Navigate into the project directory, for example [projects home]/suggestion.
 . Change the settings in `suggestion.yml` to have your application run on a
   port your choosing.
 . Run command `gradle build`.
 . Run command `gradle fatJar`.
 . Run command `gradle run`.
 . Once all three of the services have been started -- activities, forecast and
   suggestion -- you can verify installation by navigating your web browser to
   `http://localhost:9000/suggestions/{high}/{low}/{precip}/{wind}/{maxwind}/{humidity}/{maxhumidity}`,
   being sure to use your port settings, if changed, and substituting appropriate values for
   the path variables.
